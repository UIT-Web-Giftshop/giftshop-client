    ...
    <v-expansion-panel-content>
      <v-layout column>
        <v-layout column ma-0 ml-2 mb-2
          ...
          >
            <v-flex md3 sm4 xs4>
              <v-img height="100%" :src="product.thumbnail"></v-img>
            </v-flex>
            ...
            <v-flex md6 sm12 xs12 column>
              ...
              >
              <v-layout my-1 justify-space-between align-center
                ><span>Giá</span><span>{{ product.price }}</span></v-layout
              >
              <v-layout my-1 justify-space-between align-center
                ><span>Số lượng</span
        ...
        <v-layout column ma-0 ml-2 my-4
          ><span class="text-h6 text-uppercase">Thông tin đơn hàng</span>
          <v-layout column my-4 ml-8 mr-0>
            <v-layout my-1 justify-space-between align-center
              ><span>Tên</span
              ><span>{{ orderedItem.delivery_info.name }}</span></v-layout
            >
            <v-layout my-1 justify-space-between align-center
              ><span>Địa chỉ</span
            ...
            <v-layout my-1 justify-space-between align-center
              ...
              ><span>{{
                orderedItem.delivery_info.contact_number
              }}</span></v-layout
            >
        ...
        <v-layout mt-4 ml-2>
          <v-row no-gutters wrap align="center" justify="space-between">
            ...
            <v-col v-if="orderedItem.status === 'canceling'" md="2" sm="5"
              ><v-btn width="100%" class="warning">Giữ</v-btn></v-col
            >
            <v-col v-if="orderedItem.status === 'progressing'" md="2" sm="5"
              ><v-btn width="100%" class="error">Hủy</v-btn></v-col
            ></v-row
          >
        </v-layout>
      </v-layout>
